...
public final class Main {
  ...
  public static void main(String... args) {
    ...
    if (server) {
      ...
      threadCounterEntry.setDouble(9);
        ...
        if (numberImages < 1) {
          ...
          synchronized (imgLock) {
            for (int i = 0; i < numberImages; i++) {
              if (Imgproc.contourArea(pipeline.filterContoursOutput().get(i)) > maxSize) {
                maxSize = Imgproc.contourArea(pipeline.filterContoursOutput().get(i));
                maxSizeIndex = i;
              }

              Rect r = Imgproc.boundingRect(pipeline.filterContoursOutput().get(maxSizeIndex));
              rectHeight = r.height;
              rectWidth = r.width;
              x = r.x;
              y = r.y;
      ...
      addTargeting = new Thread(() -> {
        ...
        while (!Thread.interrupted()) {
          ...
          if (numberImages > 0) {
            ...
            // Imgproc.line(mat, new Point(0, imageHeight / 2), new Point(imageWidth - 1, imageHeight / 2), yellow, 2);
